...
workers=$(Rscript -e '
  ...
  stopifnot(
    ...
    identical(
      x$compute_override$provider,
      "AWS_ONLY"
    ),
    isFALSE(x$compute_override$gcp_allowed)
  )
  cat(x$compute_override$authorized_worker_count)
' "${authority}")
if [[ "${workers}" -lt 1 || "${workers}" -gt 30 ]]; then
  echo "BLOCKED: invalid worker count ${workers}." >&2
  exit 5
fi
...
awk -F, 'NR > 1 {gsub(/\r/, "", $1); print $1}' "${design}" |
  xargs -I{} -P "${workers}" bash -c 'run_cell "$1"' _ {}
...
echo "PASS: Phase 16 completed 24/24 cells with ${workers} workers."
